...
export default function HunterSignup({ getSessionv1 }: any) {
  const { data: session } = useSession();
  const { register, watch, handleSubmit } = useForm();
  const [processing, setProcessing] = useState(false);
  const watchOccupation = watch("occupation", "Developer");

  const hunterSignup = trpc.hunter.signup.useMutation();
  ...
  const onSubmit = async (data: any) => {
    type Input = inferProcedureInput<AppRouter["hunter"]["signup"]>;
    ...
    try {
      await hunterSignup.mutateAsync(input);
